...
ggplot(mpg, aes(cty, hwy)) + 
  geom_jitter(alpha = 0.5, size = 1)+ # jitter allows you to present all the data 
  geom_smooth(method = "lm")+ # lm creates a line of best fit
  theme_bw()
```
...
gapminder %>% 
  # group_by(country) %>% 
  ggplot(aes(year, lifeExp, group = country, colour = country == "Rwanda")) + # Making Rwanda a different colour 
  geom_line(alpha = 1/3)+
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  facet_wrap(~continent, scales = "free_y") # facet wrap is good for separating graphs
```
...
gapminder %>% 
  ...
  scale_x_log10(labels = scales::comma_format())+
  scale_size_area() # good for changing the size of the area
```
