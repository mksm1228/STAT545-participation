...
library(tidyverse)
library(dplyr)
library(forcats)  # for putting things into factors
library(ggplot2)
...
gapminder %>% 
  filter(year == 1997) %>% 
  mutate(life_level = case_when(lifeExp < 23 ~ "very low", # assigning anything in lifeExp less than 23 to a factor called very low
                                lifeExp < 48 ~ "low",
                                lifeExp < 59 ~ "moderate",
                                lifeExp < 70 ~ "high",
                                TRUE ~ "very high")) %>% 
...
- How many levels? What are they? 5 levels, "Africa"   "Americas" "Asia"     "Europe"   "Oceania" 
...
```{r}
gap <- gapminder %>%
  filter(country == "Canada" | country == "Brazil" | country == "Japan" | country == "China" | country == "Taiwan")
