...
```{r}
(mauna <- tsibble::as_tsibble(co2) %>% # putting co2 concentrations into a tsibble
...
```{r, fig.width = 5, fig.height = 2}
p <- ggplot(mauna, aes(month, conc)) + # plotting month vs conc 
  geom_line()+
  xlab("Month")+
  ylab("[CO2]")
```
...
ggplot(mauna, aes(y = month)) +
  geom_line(aes(x = month, y = conc))+ # putting month back on to the x-axis 
  ylab("CO2")
```
...
```{r, fig.width = 5, fig.height = 2}
p +
    geom_point(colour = "green", alpha = 0.2) # changing the color to green
...
```{r, fig.width = 5, fig.height = 2}
ggplot(gapminder) +
  geom_point(x = gdpPercap, y = lifeExp, alpha = 0.1) # fixed the code
```
