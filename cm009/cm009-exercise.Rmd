...
```{r}
(lotr_wide <- lotr %>% 
  pivot_wider(id_cols = c(-Race, -Words), # - = anything but race and words 
              names_from = Race, 
              values_from = Words))
```
...
```{r}
lotr_wide %>% 
  pivot_longer(cols = c(-Film, -Gender),  # anything but film and gender 
               names_to  = "Race", 
               values_to = "Words")
```
...
```{r}
(guest_long <- guest %>% 
  pivot_longer(cols      = c(-party, -name), # anything but party and name 
               names_to  = c(".value", "event" ), # .value preserves value 
               names_sep = "_"))
```
...
```{r}
guest_long %>% 
  separate(name, into = c("first", "last"), sep = " ") %>% # separates first and last 
