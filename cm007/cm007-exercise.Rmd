...
gapminder %>% 
  ...
  arrange(min_life) # plotting the minlifeExp with both the continent and country 
```
...
gapminder %>% 
  ...
  DT::datatable() # sorting by year first cleans the data
```
...
gapminder %>% 
  ...
  drop_na() %>% 
  ungroup() %>% # ungrouping the variables so we can group_by another variable
  group_by(continent) %>% 
