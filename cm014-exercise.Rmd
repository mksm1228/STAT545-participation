...
```{r}
(my_lm <- lm(lifeExp~year,data = France))
```
...
```{r}
(my_lm <- lm(lifeExp ~ I(year-1952), data = France))
```
...
```{r}
France2 <- data.frame(year=seq(2000,2005))
predict(my_lm,newdata = France2)
```
...
```{r}
ggplot(Zimbabwe, aes(year,lifeExp)) +
  geom_point()+
  geom_smooth(method = "lm", se = F)
```
