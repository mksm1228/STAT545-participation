...
```{r}
(time <- as_tibble(songs) %>%  # putting songs into a tibble and then putting it into a variable called time
   rename(song = title))
```
...
(album <- as_tibble(locations) %>% 
   ...
   rename(album = release, # rename the columns
          song  = title))
```
...
```{r}
time %>% 
  left_join(album, by = c("song", "artist_name")) # joins the second table from the left
```
...
```{r}
time %>% 
  anti_join(album, by = "artist_name") # finds which artists are in time, but not in album
```
